...
export async function getTenantFromRequest(req: NextRequest) {
  ...
  if (raw.startsWith("slug:")) {
    ...
    return db.tenant.findUnique({ where: { domain_slug: slug, is_active: true } })
  }

  if (raw.startsWith("custom:")) {
    const domain = raw.slice(7)
    return db.tenant.findFirst({ where: { custom_domain: domain, is_active: true } })
  }

  return db.tenant.findUnique({ where: { id: raw, is_active: true } })
}
